...
from sentense_similarity.backend.config import (
    database_name, article_collection_name, sentences_collection_name
)
from sentense_similarity.backend.article_model import ArticleInDB, ArticleInCreate
from sentense_similarity.backend.sentence_model import SentenceInDB, Sentence
from sentense_similarity.backend.text_utils import preprocess
...
async def create_sentences(conn: AsyncIOMotorClient, body: str, slug: str) -> List[Sentence]:
    ...
    for sentence in tokenize.sent_tokenize(body):
        ...
        await conn[database_name][sentences_collection_name].insert_one(
            sentence_dict
        )
...
async def fetch_all_articles(conn: AsyncIOMotorClient) -> List[ArticleInDB]:
    articles = []
    rows = conn[database_name][article_collection_name].find().sort("created_at", pymongo.DESCENDING)
    async for row in rows:
        articles.append(ArticleInDB(
            **row,
